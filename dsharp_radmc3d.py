...
def create_tables(a, lam, fm_ice, filename, extrapol):
    ...
    res_l = opacity.get_smooth_opacities(a*1e-4, lam*1e-4, rho_s, dc, smoothing='linear', n_angle=180, extrapolate_large_grains=extrapol)

    with open('{}-{}.pickle'.format(filename, name), 'wb') as fid:
        pickle.dump(res_l, fid)


def dustkappa(a, amax, lam, fm_ice, filename, extrapol):
    ...
    dc,rho_s = opacity.get_dsharp_mix(fm_ice=fm_ice)
    res = opacity.get_smooth_opacities(a*1e-4, lam*1e-4, rho_s, dc, smoothing='linear', n_angle=180, extrapolate_large_grains=extrapol)
...
def dustkapscatmat(a, amax, lam, fm_ice, filename, extrapol):
...
if __name__ == "__main__":
    ...
    parser.add_argument('-extrapol', '--extrapol', type=bool,
						help='# True or False. Extrapolate for large grains to reduce computation time. Default is True')
    ...
    if args.fig == 'only':
        ...
        if args.scatmat is not None:
            if args.scatmat == True:
                dustkapscatmat(a, amax, lam, fm_ice, filename, extrapol)
            if args.scatmat == False: 
                dustkappa(a, amax, lam, fm_ice, filename, extrapol)
        else:
            dustkappa(a, amax, lam, fm_ice, filename, extrapol)
